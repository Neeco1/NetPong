...
void cppp_pollData(uint32_t *u32Data){
...
uint16_t cppp_map(uint16_t x, uint16_t in_min, uint16_t in_max, uint16_t out_min, uint16_t out_max){
...
uint16_t cppp_readTouchX(void){
  ...
  uint16_t result = cppp_map(xCorrected, TOUCHXMIN, TOUCHXMAX, 0, 480);
  return result;
}
...
uint16_t cppp_readTouchY(void){
  ...
  uint16_t result = cppp_map(yCorrected, TOUCHYMIN, TOUCHYMAX, 0, 320);
  return result;
}
